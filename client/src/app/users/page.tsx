...
import { useConvertUserMutation, useGetUsersQuery } from '../../redux/api/users';
        ...
        <div className="pt-24 px-6">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full bg-white border border-gray-300 shadow-lg rounded-lg overflow-hidden">
                ...
                <tr className="bg-[#B5EAD7] text-left">
                  ...
                  <th className="py-4 px-6 border-b font-semibold text-[#17271e]">Role</th>
                  {
                    loggedUserRole === 'ADMIN' && (
                      <th className="py-4 px-6 border-b font-semibold text-[#17271e]">Convert</th>
                    )
                  }
                </tr>
              </thead>
              <tbody>
                {data?.map((user: User) => (
                  <tr key={user.sub} className="hover:bg-[#fdece1] transition-colors duration-300 ease-in-out">
                    ...
                    <td className="py-4 px-6 border-b text-[#17271e]">{user.role}</td>
                    {
                      loggedUserRole === 'ADMIN' && (
                        <td className="py-4 px-6 border-b text-[#17271e]">
                          <button
                          ...
                          >
                            {user.role === 'USER' ? 'Convert to Admin' : 'Convert to User'}
                          </button>
                        </td>
                      )
                    }
