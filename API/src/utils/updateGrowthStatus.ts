...
export async function updateGrowthStatus(io: any) {
  ...
  for (const slot of slots) {
    ...
    const timePlanted = slot.plantingTime
      ? new Date(slot.plantingTime).getTime()
      : now;
    ...
    if (
      ...
      timeSinceLastWatered >= 2 * 60 * 60 * 1000
    ) {
    ...
    if (
      slot.seedRarity &&
      Object.values(Rarity).includes(slot.seedRarity as Rarity)
    ) {
      harvestTime = rarityHarvestTimes[slot.seedRarity as Rarity];
    } else {
      // Manejo del caso cuando seedRarity es null o inválido
      console.log(
        `Slot with ID ${slot.id} has null or invalid seedRarity: ${slot.seedRarity}.`,
      );
