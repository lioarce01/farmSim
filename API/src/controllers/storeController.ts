...
} from '../utils/waterGeneration.js';
import { getImageForRarity } from '../utils/uploadImages.js';
...
function getRandomRarity(): Rarity {
  ...
  for (const [rarity, probability] of Object.entries(rarityProbabilities) as [
    Rarity,
    number,
  ][]) {
    sum += probability;
...
async function seedStoreWithRandomSeeds() {
  ...
  for (let i = 0; i < seedsToCreate; i++) {
    ...
    const wateringCanUrl = getImageForWater();
